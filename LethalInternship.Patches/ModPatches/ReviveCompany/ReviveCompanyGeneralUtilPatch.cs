...
namespace LethalInternship.Patches.ModPatches.ReviveCompany
{
    ...
    public class ReviveCompanyGeneralUtilPatch
    {
        ...
        static bool RevivePlayer_Prefix(int playerId)
        {
            ...
            {
                PluginLoggerHook.LogError?.Invoke($"Revive company with LethalInternship: error when trying to revive intern, could not find body.");
                return false;
            }
            ...
            {
                PluginLoggerHook.LogError?.Invoke($"Revive company with LethalInternship: error when trying to revive intern \"{internIdentity.Name}\", intern is already alive! do nothing more");
                return false;
            }

            // Update remaining revives
            InternManagerProvider.Instance.UpdateReviveCompanyRemainingRevivesServerRpc(internIdentity.Name);
