...
namespace LethalInternship.Patches.MapHazardsPatches
{
    ...
    {
        static MethodInfo DamagePlayersInLOSMethod = SymbolExtensions.GetMethodInfo(() => DamagePlayersInLOS(new Turret()));
        ...
        public static IEnumerable<CodeInstruction> Update_Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            {
                PluginLoggerHook.LogError?.Invoke($"LethalInternship.Patches.MapHazardsPatches.TurretPatch.Update_Transpiler use other method for shooting player/intern in TurretMode.Firing");
            }
            ...
            {
                PluginLoggerHook.LogError?.Invoke($"LethalInternship.Patches.MapHazardsPatches.TurretPatch.Update_Transpiler use other method for shooting player/intern in TurretMode.berzerk");
            }
        ...
        private static PlayerControllerB? DamagePlayersInLOS(Turret turret)
        {
            ...
            IInternAI? internAI = InternManagerProvider.Instance.GetInternAIIfLocalIsOwner((int)player.playerClientId);
            if (internAI == null)
            {
                // Player not intern
                return player;
            ...
            {
                PluginLoggerHook.LogDebug?.Invoke($"SyncKillIntern from turret, intern object: Intern #{internAI.NpcController.Npc.playerClientId}");
