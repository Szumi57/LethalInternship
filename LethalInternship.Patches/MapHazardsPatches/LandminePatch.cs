...
namespace LethalInternship.Patches.MapHazardsPatches
{
    ...
    public class LandminePatch
    {
                                           ...
                                           int nonLethalDamage)
        {
            ...
            {
                PluginLoggerHook.LogDebug?.Invoke($"SpawnExplosion OverlapSphere array {i} {array[i].name}");
                ...
                internAI = InternManagerProvider.Instance.GetInternAIIfLocalIsOwner((int)internController.playerClientId);
                if (internAI == null)
                {
                    continue;
                }
                ...
                if (distanceFromExplosion < killRange)
                {
                    ...
                    PluginLoggerHook.LogDebug?.Invoke($"SyncKillIntern from explosion, intern object: Intern #{internAI.NpcController.Npc.playerClientId}");
