...
namespace LethalInternship.Patches.NpcPatches
{
    ...
    public class EnemyAIPatch
    {
        ...
        {
            PluginLoggerHook.LogDebug?.Invoke($"Try ChangeOwnershipOfEnemy newOwnerClientId : {(int)newOwnerClientId}");
            if (newOwnerClientId > Const.INTERN_ACTUAL_ID_OFFSET)
            {
                ...
                if (internAI == null)
                {
                    PluginLoggerHook.LogDebug?.Invoke($"Could not find intern with id : {(int)(newOwnerClientId - Const.INTERN_ACTUAL_ID_OFFSET)}, aborting ChangeOwnershipOfEnemy.");
                    return false;
                }

                PluginLoggerHook.LogDebug?.Invoke($"ChangeOwnershipOfEnemy not on intern but on intern owner : {internAI.OwnerClientId}");
        ...
        {
            if (__instance.GetType() == typeof(IInternAI))
            {
                return false;
            }
        ...
        public static IEnumerable<CodeInstruction> MeetsStandardPlayerCollisionConditions_Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            {
                PluginLoggerHook.LogError?.Invoke($"LethalInternship.Patches.NpcPatches.EnemyAIPatch.MeetsStandardPlayerCollisionConditions_Transpiler could not insert instruction if is intern for \"component != GameNetworkManager.Instance.localPlayerController\".");
            }
        ...
        static void CheckLineOfSightForPlayer_PostFix(EnemyAI __instance, ref PlayerControllerB __result, float width, ref int range, int proximityAwareness)
        {
            ...
            for (int i = InternManagerProvider.Instance.IndexBeginOfInterns; i < StartOfRound.Instance.allPlayerScripts.Length; i++)
            {
                ...
                if (to.sqrMagnitude < range * (float)range
                    && !Physics.Linecast(__instance.eye.position, position, StartOfRound.Instance.collidersAndRoomMaskAndDefault))
                {
                    if (Vector3.Angle(__instance.eye.forward, to) < width || proximityAwareness != -1 && Vector3.Distance(__instance.eye.position, position) < proximityAwareness)
                    {
