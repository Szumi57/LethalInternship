...
namespace LethalInternship.Patches.EnemiesPatches
{
    ...
    public class FlowerSnakeEnemyPatch
    {
        ...
        public static IEnumerable<CodeInstruction> OnCollideWithPlayer_Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            {
                PluginLoggerHook.LogError?.Invoke($"LethalInternship.Patches.EnemiesPatches.FlowerSnakeEnemyPatch.OnCollideWithPlayer_Transpiler could not use id player local or intern");
            }
                                                    ...
                                                    ref Vector3 ___forces)
        {
            ...
            IInternAI? internAI = InternManagerProvider.Instance.GetInternAI((int)__instance.clingingToPlayer.playerClientId);
            if (internAI == null)
            {
                return;
            }
            ...
            {
                __instance.flightPower = Mathf.Clamp(__instance.flightPower + Time.deltaTime * 1.7f, 12f, __instance.clingingToPlayer.enemiesOnPerson * 4f);
            }
            else
            {
        ...
        {
            if (InternManagerProvider.Instance.IsIdPlayerInternOwnerLocal(playerId)
