...
namespace LethalInternship.Patches.EnemiesPatches
{
    ...
    public class MaskedPlayerEnemyPatch
    {
        ...
        static bool FinishKillAnimation_PreFix(MaskedPlayerEnemy __instance)
        {
            ...
            IInternAI? internAI = InternManagerProvider.Instance.GetInternAI((int)__instance.inSpecialAnimationWithPlayer.playerClientId);
            if (internAI == null)
            {
                return true;
            }
        ...
        public static IEnumerable<CodeInstruction> KillPlayerAnimationClientRpc_Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            {
                PluginLoggerHook.LogError?.Invoke($"LethalInternship.Patches.EnemiesPatches.MaskedPlayerEnemyPatch.KillPlayerAnimationClientRpc_Transpiler could not check if player local or intern local 1");
            }
            ...
            {
                PluginLoggerHook.LogError?.Invoke($"LethalInternship.Patches.EnemiesPatches.MaskedPlayerEnemyPatch.KillPlayerAnimationClientRpc_Transpiler could not check if player local or intern local 2");
            }
