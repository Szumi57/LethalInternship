...
namespace LethalInternship.Patches.EnemiesPatches
{
    ...
    public class ButlerEnemyAIPatch
    {
        ...
        public static IEnumerable<CodeInstruction> OnCollideWithPlayer_Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            {
                PluginLoggerHook.LogError?.Invoke($"LethalInternship.Patches.EnemiesPatches.ButlerEnemyAIPatch.OnCollideWithPlayer_Transpiler could not check if local player or intern owner local player");
            }
        ...
        {
            ___lastSeenPlayerPositions = new Vector3[InternManagerProvider.Instance.AllEntitiesCount];
            ___seenPlayers = new bool[InternManagerProvider.Instance.AllEntitiesCount];
            ___timeOfLastSeenPlayers = new float[InternManagerProvider.Instance.AllEntitiesCount];
        }
    }
}
