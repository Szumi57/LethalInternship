...
namespace LethalInternship.Patches.EnemiesPatches
{
    ...
    public class CrawlerAIPatch
    {
        ...
        {
            ___nearPlayerColliders = new Collider[InternManagerProvider.Instance.AllEntitiesCount];
        }
        ...
        public static IEnumerable<CodeInstruction> OnCollideWithPlayer_Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            {
                PluginLoggerHook.LogError?.Invoke($"LethalInternship.Patches.EnemiesPatches.CrawlerAIPatch.OnCollideWithPlayer_Transpiler could not change use of correct player id for HitPlayerServerRpc.");
            }
            ...
            {
                PluginLoggerHook.LogError?.Invoke($"LethalInternship.Patches.EnemiesPatches.CrawlerAIPatch.OnCollideWithPlayer_Transpiler could not change use of correct player id for JumpToFearLevel.");
            }

            // ----------------------------------------------------------------------
            //PluginLoggerHook.LogDebug?.Invoke($"OnCollideWithPlayer ======================");
            //for (var i = 0; i < codes.Count; i++)
            //{
            //    PluginLoggerHook.LogDebug?.Invoke($"{i} {codes[i].ToString()}");
            //}
            //PluginLoggerHook.LogDebug?.Invoke($"OnCollideWithPlayer ======================");
        ...
        public static IEnumerable<CodeInstruction> Update_Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            {
                PluginLoggerHook.LogError?.Invoke($"LethalInternship.Patches.EnemiesPatches.CrawlerAIPatch.Update_Transpiler could not change condition for is only local player.");
            }
            ...
            {
                PluginLoggerHook.LogError?.Invoke($"LethalInternship.Patches.EnemiesPatches.CrawlerAIPatch.Update_Transpiler could not change target of begin chasing player.");
            }
